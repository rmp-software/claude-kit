...
For monorepos, detect affected packages from git rather than rebuilding everything:
```bash
git diff --name-only HEAD | grep -E '^(services|packages|apps)/' | cut -d/ -f2 | sort -u
```
If the project tracks edited files some other way (a session cache, a task list), use that. Build only affected packages to save time.
